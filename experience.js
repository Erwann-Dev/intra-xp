...
const getAllExperiences = async (activities, region, login) => {
    ...
    try {
        ...
        for (let i = 0; i < res.length; i++) {
            if (res[i].registered.length > 0 && res[i].project_title.includes("Experience")) {
                if (res[i].registered[0].master.login === login) {
                    addActivite(res[i].project_title, 'Experience', 'present', res[i].begin);
                }
            }
        }
...
const getXp = async () => {
    ...
    value.innerHTML =
        lang === 'fr' ? `Validées : ${me.nbXps}
        <br> En cours : ${me.nbXpsSoon}
        ...
        <br> Experience: ${participation.experience} (+${participation.experiencesoon}) / 8
        <br> Total: ${me.nbXps + me.nbXpsSoon} / 50` :
            `Validated : ${me.nbXps}
        <br>In progress : ${me.nbXpsSoon}
        <br> Talk: ${participation.talk} (+${participation.talksoon}) / 15
        <br> Workshop: ${participation.workshop} (+${participation.workshopsoon}) / 10
        <br> Hackathon: ${participation.hackaton} (+${participation.hackatonsoon})
        <br> Experience: ${participation.experience} (+${participation.experiencesoon}) / 8
